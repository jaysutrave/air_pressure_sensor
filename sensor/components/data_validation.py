...
class data_validation:
    ...
    def drop_missing_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """ this function will  drop the Columns which contain then missing  value more than sepcified threshold 
        
        df:Accepts a pandas datafrom
        threshold :percentage crietra to drop a columns
        ================================================================
        return pandas DataFrame
        if atleast a single columns is available after missing columns drop
        """
        try:
            
            threshold = self.Data_validation_config.missing_threshold
            null_report =  df.isna().sum()/df.shape[0]
            #select columns which is having high missing values i.e greater than 0.3%
            logging.info(f"select columns which is having high missing values i.e greater than {threshold}")
            drop_columns_name=null_report[null_report>threshold].index
            logging.info(f"coloumns to be dropped {list(drop_columns_name)}")
        ...
        try:
            ...
            for base_column in base_columns:
                if base_column not in current_columns:
                    logging.info(f"coloumns :[ {base} is not available.")
    ...
    def data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                same_distrubtion=k2_samp(base_data,current_data)
                if same_distrubtion.pvalue>0.05:
                    #we are accpecting the null hypothesis
                    drift_report[base_column]={
                        "pvalue":float(same_distribution.pvalue),
                        "same_distrubtion":True
                    }
                else:
                    #diffrent distrubtion
                    drift_report[base_column]={
                        "pvalue":float(same_distribution.pvalue),
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"replace na value with base df")
            #base has na as null
            logging.info(f"drop nulll columns from base df")
            ...
            if train_df_columns_status:
                logging.info(f"as all columns available train df hence detecting data drift") 
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name="data_drift_within_train_dataset")
            if test_df_columns_status:
                logging.info(f"as all columns available test df hence detecting data drift") 
                self.data_drift(base_df=base_df, current_df=test_df,report_key_name="data_drift_within_test_dataset")

            #write the reprot
            logging.info(f"wiritng report in yaml file ") 
